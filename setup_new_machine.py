...
SYMLINKS = {
    ...
    "$HOME/dotfiles/local_profile_extensions": "$HOME/.local_profile_extensions",
...
def check_python_version():
    ...
    proc = subprocess.Popen(
        ["python", "-V"], stderr=subprocess.PIPE, stdout=subprocess.PIPE
    )
...
def install_google_cloud_sdk():
    ...
    if still_valid.lower() != "y":
        msg = (
            "Please change install_google_cloud_sdk() to reflect the\n"
...
def change_system_paths():
    ...
    if system_path_contents == custom_path_contents:
        ...
        print(
            "%r->%r mapping already exists."
            % (system_path_file, custom_path_file)
        )
        return
...
def homebrew_install():
    proc = subprocess.Popen(
        ["which", "brew"], stderr=subprocess.PIPE, stdout=subprocess.PIPE
    )
    ...
    if proceed.lower() != "y":
        msg = (
            "Please change homebrew_install() to reflect the current\n"
...
def install_from_pkg_link(link):
    ...
    if proceed.lower() != "y":
        msg = (
            "Please change the relevant method to reflect the current\n"
            "recommended way to install."
        )
        raise ValueError(msg)
...
def nodejs_install():
    proc = subprocess.Popen(
        ["which", "node"], stderr=subprocess.PIPE, stdout=subprocess.PIPE
    )
    stdout, _ = proc.communicate()
    if stdout != "":
...
def install_tex_on_mac():
    proc = subprocess.Popen(
        ["which", "pdflatex"], stderr=subprocess.PIPE, stdout=subprocess.PIPE
    )
    stdout, _ = proc.communicate()
    if stdout != "":
...
def replace_line(lines, old_line, new_line):
    old_line_matches = [
        (i, line) for i, line in enumerate(lines) if line == old_line
    ]
...
def _linux_make_ssh_public_key_only():
    ...
    replaced_pw = replace_line(
        lines, "#PasswordAuthentication yes", "PasswordAuthentication no"
    )
...
def _os_x_make_ssh_public_key_only():
    ...
    replaced_pw = replace_line(
        lines, "#PasswordAuthentication no", "PasswordAuthentication no"
    )
...
def matplotlib_suggestion(index):
    ...
    try:
        ...
        rc_file = os.path.join(
            os.path.dirname(matplotlib.__file__), "mpl-data", "matplotlibrc"
        )
    except ImportError:
        pass
